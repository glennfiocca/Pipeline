...
if (!serverCode.includes('process.env.PORT')) {
  ...
  console.error('Please update your server code to use: const PORT = process.env.PORT || 5000;');
  process.exit(1);
}

// Create a .port-config.json file if it doesn't exist
const portConfigPath = path.join(__dirname, '.port-config.json');
const portConfig = { port: 5000 };
...
console.log('Your application is now configured to use PORT=5000');
